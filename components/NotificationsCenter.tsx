...
export default function NotificationsCenter({ accentColor = '#1d3161' }: { accentColor?: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-[var(--card)] border border-[var(--border)] rounded-lg shadow-lg z-50">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
            <span className="font-semibold text-[var(--foreground)] text-sm">Notifications</span>
            {unreadCount > 0 && (
              <button onClick={markAllRead} className="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
                Mark all read
              </button>
            )}
          </div>
          ...
          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              ...
              notifications.map(n => {
                ...
                return (
                  <Link
                    ...
                    className={`block px-4 py-3 border-b border-[var(--border-subtle)] hover:bg-[var(--muted)]/50 transition-colors ${
                      !n.is_read ? 'bg-blue-50/50 dark:bg-blue-500/10' : ''
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${config.color} dark:opacity-80`}>
                        {config.icon}
                      </span>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-[var(--foreground)]">{n.title}</div>
                        {n.message && <div className="text-xs text-[var(--muted-foreground)] truncate">{n.message}</div>}
                        <div className="text-[10px] text-[var(--muted-foreground)] mt-1">
                          {new Date(n.created_at).toLocaleDateString()}
                        </div>
                      </div>
                      {!n.is_read && (
                        <span className="w-2 h-2 rounded-full flex-shrink-0 mt-1.5 bg-accent" />
                      )}
                    </div>
                  </Link>
                );
              ...
              <div className="py-8 text-center text-sm text-[var(--muted-foreground)]">No notifications</div>
            )}
          </div>
        </div>
      )}
